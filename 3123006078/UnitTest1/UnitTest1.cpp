...
namespace PlagiarismCheckerUnitTests {
    ...
    public:
        // 1. 完全相同的两段文本，相似度应为 1.00
        TEST_METHOD(TestIdenticalText) {
            std::string text1 = "今天是星期天，天气晴朗，今天晚上我要去看电影。";
            std::string text2 = text1;
            auto a = prepareInputText(text1);
            auto b = prepareInputText(text2);
        ...
        TEST_METHOD(TestEmptyBoth) {
            std::vector<CodePoint> a;
            std::vector<CodePoint> b;
            double sim = calculateSimilarity(a, b);
            Assert::AreEqual(0.0, sim, 0.001);
        }

        // 3. 一篇为空，另一篇非空，相似度应为 0.00
        TEST_METHOD(TestEmptyOne) {
            std::string text = "一位真正的作家永远只为内心写作。";
            auto a = prepareInputText(text);
            std::vector<CodePoint> b;
            double sim = calculateSimilarity(a, b);
            Assert::AreEqual(0.0, sim, 0.001);
        }
        ...
        TEST_METHOD(TestShortText) {
            std::vector<CodePoint> a = { 'a' };
            std::vector<CodePoint> b = { 'a' };
            double sim = calculateSimilarity(a, b);
            Assert::AreEqual(1.0, sim, 0.001);

            std::vector<CodePoint> c = { 'b' };
            double simDiff = calculateSimilarity(a, c);
            Assert::AreEqual(0.0, simDiff, 0.001);
        }

        // 5. 完全无关的文本，相似度应为 0.00
        TEST_METHOD(TestCompletelyDifferent) {
            std::string t1 = "白日依山尽黄河入海流";
            std::string t2 = "春眠不觉晓处处闻啼鸟";
            auto a = prepareInputText(t1);
            auto b = prepareInputText(t2);
            double sim = calculateSimilarity(a, b);
            Assert::AreEqual(0.0, sim, 0.001);
        }
        ...
        TEST_METHOD(TestFullwidthAndCase) {
            std::string t1 = "软件工程ＡＢＣ１２３";
            std::string t2 = "软件工程abc123";
            auto a = prepareInputText(t1);
            auto b = prepareInputText(t2);
            double sim = calculateSimilarity(a, b);
            Assert::AreEqual(1.0, sim, 0.001);
        ...
        // 7. 标点符号与空白字符过滤测试
        TEST_METHOD(TestPunctuationAndWhitespace) {
            std::string t1 = "今天，天气 晴；晚上，我要去 看电影！";
            std::string t2 = "今天天气晴晚上我要去看电影";
            auto a = prepareInputText(t1);
            auto b = prepareInputText(t2);
            double sim = calculateSimilarity(a, b);
            Assert::AreEqual(1.0, sim, 0.001);
        ...
        // 8. 内存 HTML 脏数据清洗与正文提取测试
        TEST_METHOD(TestHtmlCleaning) {
            std::string html = "<!DOCTYPE html><html><head><script>alert(1)</script><style>.a{color:red}</style></head><body><table><tr><td class=\"blob-code\">这是正文内容</td></tr></table></body></html>";
            std::string plain = "这是正文内容";
            auto a = prepareInputText(html);
            auto b = prepareInputText(plain);
            double sim = calculateSimilarity(a, b);
            Assert::AreEqual(1.0, sim, 0.001);
        }

        // 9. 局部增删改与调换语序的相似度区间测试
        TEST_METHOD(TestModification) {
            std::string orig = "今天是星期天，天气晴，今天晚上我要去看电影。";
            std::string plag = "今天是周天，天气晴朗，我晚上要去看电影。";
            auto a = prepareInputText(orig);
            auto b = prepareInputText(plag);
            double sim = calculateSimilarity(a, b);
            Assert::IsTrue(sim > 0.5 && sim < 1.0);
        }

        // 10. UTF-8 BOM 文件头跳过测试
        TEST_METHOD(TestUtf8Bom) {
            std::string withBom = "\xEF\xBB\xBF测试文本内容";
            std::string noBom = "测试文本内容";
            auto a = prepareInputText(withBom);
            auto b = prepareInputText(noBom);
            double sim = calculateSimilarity(a, b);
            Assert::AreEqual(1.0, sim, 0.001);
        }
    };
